...
def extract_events(feed_json: dict) -> list[dict]:
    ...
    for play in all_plays:
        ...
        # the description (e.g. a bad throw on a steal, or a passed ball
        # allowing the batter to reach after strike three) but aren't the
        # kind of fielding error worth alerting on.
        EXCLUDED_EVENTS = ("stolen base", "caught stealing", "strikeout", "pickoff")
        is_excluded_context = any(x in event_name.lower() for x in EXCLUDED_EVENTS)

        if not is_excluded_context and ("error" in event_name.lower() or " error " in description.lower()):
